...
class CapitalSerializer(serializers.ModelSerializer):
    ...
    def validate_capital_city(self, value):
        ...
        for char in value:
            if not (char.isalpha() or char in allowed_chars):
                raise serializers.ValidationError(f"Название столицы '{value}' содержит недопустимый символ '{char}'")
        return value.strip()
    ...
    def update(self, instance, validated_data):
        instance.capital_city = validated_data.get('capital_city', instance.capital_city)
        instance.capital_population = validated_data.get('capital_population', instance.capital_population)
